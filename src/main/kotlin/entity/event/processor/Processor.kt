...
import tea.ulong.entity.event.processor.annotation.RespondEventModel
import tea.ulong.entity.event.processor.annotation.RespondEvent
...
class Processor(val clazz: KClass<*>) {
    ...
    companion object {
         ...
         * 默认事件类型
         */
        var UniversalRespondEvent = listOf(
...
class ProcessorFun(val function: KFunction<*>, val processor: Processor) {
    ...
    var respondEvent: List<RespondEventModel> =
        function.findAnnotation<RespondEvent>()?.model?.toList() ?:
        Processor.UniversalRespondEvent
    ...
    suspend fun run(key:String, event: Event) {
        ...
        if (event is MessageEvent){
            // 检查key是否符合条件,这一步已经在外层实现了,未来可能删除
